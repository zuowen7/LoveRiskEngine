...
|---|---|---|---|---|
| Structured observation (observation / interpretation / **required alternative explanation**) | Separating the record from the reading, plus forcing at least one alternative explanation, reduces confirmatory assimilation of ambiguous evidence | *Consider the opposite* — Lord, Lepper & Preston (1984) | theory_supported | Live since v0.1; the only mechanism with direct experimental support for the mechanism itself |
...
| `repeated_rationalization` | Making consecutive self-justifications visible reduces the length of rationalization runs | Motivated reasoning / self-justification runs; the theory-supported countermeasure (required alternative explanations) lives in the observation schema, not this rule | engineering_heuristic | uncalibrated | — |
